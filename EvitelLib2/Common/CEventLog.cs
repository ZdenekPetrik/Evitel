    ...
    public class CEventLog
    {
        ...
        public void WriteMainEventLog(DateTime dtCreate, eEventCode e1, eEventSubCode e2, string Text, string Value, int LoginUserId)
        {
            if (ConnectionString == null)
                ConnectionString = ConfigurationManager.ConnectionStrings["DBEvitel2"].ToString();
            SqlConnection connection = new SqlConnection(ConnectionString);
            try
            {
                string queryString = "INSERT dbo.MainEventLogs ([dtCreate],[Program],[LoginUserId],[eventCode],[eventSubCode],[Text],[Value]) VALUES (@dtCreate,@Program,@LoginUserId,@eventCode,@eventSubCode,@Text,@Value)";
                ...
                command.Parameters.AddWithValue("@eventCode", e1);
                command.Parameters.AddWithValue("@eventSubCode", e2);
